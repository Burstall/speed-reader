...
export function PlaybackControls() {
  const { isPlaying, toggle, restart, goBack, goForward, words, currentIndex } = useReaderStore();
  const hasContent = words.length > 0;
  const isMidReading = currentIndex > 0 && !isPlaying;

  return (
    <div className="flex items-center justify-center gap-2 md:gap-4">
      {/* Restart - smaller and less prominent when paused mid-content */}
      <button
        onClick={restart}
        disabled={!hasContent}
        className={`
          ...
          ${isMidReading ? 'p-1.5 md:p-2 opacity-60' : 'p-2 md:p-3'}
        `}
        title="Restart (R)"
        aria-label="Restart"
      >
        <svg className={isMidReading ? 'w-4 h-4 md:w-5 md:h-5' : 'w-5 h-5 md:w-6 md:h-6'} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <button
        ...
        className="p-2 md:p-3 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800
                   disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        title="Back 10 words (←) / Sentence (Shift+←)"
      ...
      <button
        ...
        className="p-2 md:p-3 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800
                   disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        title="Forward 10 words (→) / Sentence (Shift+→)"
